...
function dataFunc(datas) {
  ...
  Object.keys(
    dataArr.map((i, j) => {
      ...
      tr1.innerHTML =
        `<td rowspan='2'>\
        <input value='${idx}' type='checkbox' name='midx[` +
        [j] +
        `]' onchange='checkFunc(this.checked)' />\
        </td>\
        <td rowspan=2' id='user_id${idx}'>${dataArr[j]["user_id"]}</td>\
        <td id='user_name${idx}'>${dataArr[j]["user_name"]}</td>\
        <td colspan='6'>\
        <label for='member_identify1${idx}'>\
        <input type='radio' name='identify' id='member_identify1${idx}' /> 아이핀\
        </label>\
        <label for='member_identify2${idx}'>\
        <input type='radio' name='identify' id='member_identify2${idx}' /> 휴대폰\
        </label>\
        </td>\
        <td id='user_hp${idx}'>${dataArr[j]["user_hp"]}</td>\
        <td>정상<select id='levelSel${idx}'>\
        ...
        <td rowspan='2' id='user_modify${idx}'>\
        <button>수정</button>\
        <button>그룹</button>\
        </td>`

      tr2.innerHTML = `<tr>\
      <td id='user_nick${idx}'>${dataArr[j]["user_nick"]}</td>\
        <td id='mail_accept${idx}'>${dataArr[j]["mail_accept"]}</td>\
